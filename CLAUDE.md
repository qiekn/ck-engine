...
| `src/editor/`       | `editor` exe (`ck_editor::Editor` Application + `ck_editor::EditorLayer` + `ck_editor::EditorCamera` + `panels/` — `ck_editor::StatsPanel`, `ck_editor::ViewportPanel`, `ck_editor::SceneHierarchyPanel`, `ck_editor::PropertiesPanel`) |
...
- **`Scene`** / **`Entity`** / **components** (`scene/scene.{h,cpp}`, `scene/entity.h`, `scene/components.h`) — `Scene` owns an `entt::registry`; `Entity` is a thin `(entt::entity, Scene*)` handle so clients add components via `entity.AddComponent<T>()` without ever seeing entt. `Scene::OnUpdate` iterates the `(TransformComponent, SpriteRendererComponent)` view and dispatches each to `Renderer2D::DrawQuad`. `Scene::Clear` wipes the registry (used by SceneSerializer::Deserialize). Components: `TagComponent` (display name), `TransformComponent` (translation/rotation/scale vec3 — rotation as XYZ-euler radians, `GetTransform()` composes T·R·S on demand via `glm::mat4_cast(glm::quat(rotation))`), `SpriteRendererComponent` (TextureHandle + texture_path string + tint).
- **`SceneSerializer`** (`scene/scene_serializer.{h,cpp}`) — yaml-cpp-backed save/load. Serialize emits `Scene/Entities` map with per-entity `TagComponent` / `TransformComponent` / `SpriteRendererComponent` blocks (vec3/vec4 in flow style). Deserialize calls `Scene::Clear`, replays entities into the bound scene, and reruns `Renderer2D::LoadTexture(texture_path)` to repopulate runtime bindless slots.
...
- **`Camera`** (`renderer/camera.{h,cpp}`) — orthographic camera with `SetViewport` / `SetPosition` / `SetZoom` / `view_projection`; `zoom` is world half-height (default 1.0; clamped to [0.01, 1000]); Vulkan Y-down NDC handled internally.
...
- **Phase 6.B** ✅ scene/ECS + editor panels — 6.B.1 ✅ Scene/Entity/components scaffolding (entt-backed) + editor switched to Scene-driven path; 6.B.2 ✅ SceneHierarchyPanel (ck_editor::, lists entities, click-select, right-click create/delete) + Scene::GetAllEntities; 6.B.3 ✅ PropertiesPanel (Tag rename / TransformComponent T/R/S split / SpriteRenderer color) + ImGui editor exports; 6.B.4 ✅ EditorCamera (middle-mouse pan + scroll-wheel zoom, gated on ViewportPanel hover) + Camera::SetZoom + Renderer::GetCamera; 6.B.5 ✅ SceneSerializer (yaml-cpp save/load, File menu in editor) + Scene::Clear + SpriteRendererComponent.texture_path
